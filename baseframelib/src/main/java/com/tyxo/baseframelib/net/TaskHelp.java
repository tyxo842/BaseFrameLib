package com.tyxo.baseframelib.net;
...
import com.tyxo.baseframelib.net.volley.VolleyManager;
...
public class TaskHelp {
    ...
    public static void testNet(Context context, String... args) {
        ...
        SharedPreferences mSpUser = context.getSharedPreferences("", Context.MODE_PRIVATE);
    ...
    public static void getGirls(Context context, int pageSize, int pageIndex, int rand, VolleyCallBack<JSONObject> callback){
        String urlBase = String.format("%s%s", "http://route.showapi.com/", "197-1");//前者为baseUrl,后为拼接.
